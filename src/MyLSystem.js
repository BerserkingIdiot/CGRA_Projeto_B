...
class MyLSystem extends CGFobject {
  ...
  iterate() {
    ...
    for (i = 0; i < this.iterations; ++i) {
      ...
      // substitui cada um dos caracteres da cadeia de caracteres de acordo com
      // as produções
      for (j = 0; j < this.axiom.length; ++j) {
        var axiomProductions = this.productions[this.axiom[j]];
        // aplicar producoes
        if (axiomProductions === undefined) {
          // caso nao se aplique nenhuma producao deixa estar o caracter
          // original
          ...
          // sistema estocastico - varias producoes sao aplicaveis - seleciona
          // aleatoriamente
          newString += axiomProductions[Math.floor(
              Math.random() * axiomProductions.length)];
        }
      }
